...
- [Кодирование](#кодирование)
  ...
  - [2023.01.21 Работа с текстовыми файлами](#20230121-работа-с-текстовыми-файлами)
  - [2023.01.28 Работа с текстовыми файлами](#20230128-работа-с-текстовыми-файлами)
  - [2023.02.04 Работа с текстовыми файлами](#20230204-работа-с-текстовыми-файлами)
  - [2023.02.11 Кодирование изображений и звука](#20230211-кодирование-изображений-и-звука)
- [Количество информации](#количество-информации)
  - [2023-02-18 Количество информации](#2023-02-18-количество-информации)
  - [2023-03-04 Количество информации](#2023-03-04-количество-информации)
  - [2023-03-11 Количество информации. Задачи](#2023-03-11-количество-информации-задачи)
  - [2023-03-18 Разбор контрольной работы](#2023-03-18-разбор-контрольной-работы)
  - [2023-04-08 Практика. Подсчёт количества информации в файле](#2023-04-08-практика-подсчёт-количества-информации-в-файле)
  - [2023-04-15 Разборы контрольной и практики](#2023-04-15-разборы-контрольной-и-практики)
  - [2023-04-22 Решаем пробник ОГЭ](#2023-04-22-решаем-пробник-огэ)
  - [2023-05-13 Алгоритмы построения оптимального префиксного кодирования](#2023-05-13-алгоритмы-построения-оптимального-префиксного-кодирования)
- [Программа зачёта](#программа-зачёта)
...
**Определение.** Пусть $q\in\mathbb{N}$, $a\in\mathbb{R}$. Представление
...
Для чисел из промежутка $[0; 2^{16})$ используют два последовательных байта (`WORD`). И так далее до `QWORD`.

Важно понимать: если процессор будет складывать два числа, упакованных в `WORD`, а результат сложения окажется $2^{16}$ или больше, то старшие цифры числа процессор отбросит, так как ему некуда будет их записать. Так, например, $10010100_2 + 10111100_2 = 01010000_2$ при упаковке в `WORD`.

Для кодирования чисел, выходящих за пределы промежутка $[0;2^{64})$ используют массивы: число представляют в системе счисления с основанием $2^{64}$ и полученные цифры записывают в массив. Процессор не умеет работать с такими числами напрямую, поэтому для оперирования такими большими числами пишутся отдельные программы по типу тех, что мы писали ранее.
...
`NaN` - не число. Появляется как результат математических операций с неопределённым результатом. Например, в результате деления `0/0` или извлечения квадратного корня из `-1`. В `float32` кодируется как `01111111110000000000000000000000`.

Все операции сравнения с `NaN` дают `false`. Все арифметические операции с `NaN` дают `NaN`.

#### INFINITY

`INFINITY`, $+\infty$, - выделенное значение для обозначения плюс бесконечности. В `float32` кодируется как `01111111100000000000000000000000`. Бесконечность бывает положительная и отрицательная.
...
### 2023.01.21 Работа с текстовыми файлами

#### Правила при работе с текстовыми файлами

1. При работе с текстовыми файлами используйте стандартную конструкцию try with resources вашего языка программирования. Например в python эта конструкция начинается с ключевого слова `with`:
...
Данная конструкция гарантирует, что файл `fin` будет закрыт сразу как только закончится выполнение блока `with` и не важно по какой причине этот блок завершил свою работу: в блоке закончились инструкции, произошла исключительная ситуация, произошел выход из функции по `return` и т.п. Данный фрагмент кода является сокращением от:

```Python
fin = open("example.txt", "r", encoding="utf-8") as fin:
try:
    # Работа с файлом fin
finally:
    fin.close()
...
2. При открытии текстового файла на чтение или на запись всегда указывайте кодировку: `encoding="utf-8"`. И если это зависит от вас, то всегда используйте `UTF-8`.

3. Большие текстовые файлы лучше читать построчно, так как при чтении их целиком (через `fin.read()`) они могут не поместиться в оперативной памяти.

#### Задание

В текстовом файле [`input.csv`](2023-01-21/input.csv) записана таблица с данными.

- Скачайте этот файл на свой компьютер.
- Просмотрите содержимое файла в текстовом редакторе и изучите его содержимое.
- Откройте этот файл в программе LibreOffice Calc и изучите его содержимое.
- Напишите программу, которая:
  - читает таблицу из данного файла;
  - вычисляет среднее арифметическое чисел в каждой колонке;
  - вычисляет число строк таблицы;

### 2023.01.28 Работа с текстовыми файлами
...
- Напишите программу, которая:
  ...
  - вычисляет среднее арифметическое чисел в каждой колонке;
  - вычисляет число строк таблицы;
  - спрашивает у пользователя числа $a$ и $b$ и вычисляет значение величины $d_i = |z_i - (a\cdot x_i + b\cdot y_i)|$ для каждой строки $i$ таблицы;
  - вычисляет среднее значение $d$;
  - записывает в файл `output.csv` таблицу с колонками $x$, $y$, $z$, $d$.
...
$$k_{xy} = (x_1\cdot y_1 + ... + x_n\cdot y_n)/n$$

$$k_{xz} = (x_1\cdot z_1 + ... + x_n\cdot z_n)/n$$

$$k_{yz} = (y_1\cdot z_1 + ... + y_n\cdot z_n)/n$$

Найдите коэффициенты $a$ и $b$ из системы линейных уравнений

$$a\cdot k_{xx} + b\cdot k_{xy} = k_{xz},\quad a\cdot k_{xy} + b\cdot k_{yy} = k_{yz},$$

посчитайте величину $d_i = |z_i - (a\cdot x_i + b\cdot y_i)|$ и среднее значение величины $d$.

Поздравляю вы только что решили задачу линейной регрессии.

### 2023.02.04 Работа с текстовыми файлами

Изучите функции работы со строками:
...
- спрашивает у пользователя некоторый фрагмент в виде строки;
- выводит на экран сколько раз данный фрагмент встречается в файле.

Для того, чтобы найти подстроку `substr` в строке `str`, начиная с символа с номером `start` можно использовать функцию

```Python
idx = str.find(substr, start)
```

Если в строке `str` после символа с номером `start` встречается подстрока `substr` то эта функция вернёт номер первого символа, с которого начинается подстрока `substr`. В противном случае функция `find` вернёт `-1`. Например:
...
- спрашивает у пользователя имя файла;
- спрашивает у пользователя некоторый фрагмент в виде строки;
- выводит на экран сколько раз данный фрагмент встречается в файле как фраза.

Будем считать что некоторая подстрока является *фразой*, если в основной строке слева и справа от этой подстроки стоят не буквы. Например, подстрока `"мама мыла"` является фразой в строке `"мама мыла электрочайник"`, а подстрока `"чайник"` в данной строке фразой не является.

Для того, чтобы определить, что некоторый символ является буквой какого-то алфавита, можно использовать функцию `isalpha`. Например:
...
### 2023.02.11 Кодирование изображений и звука
...
Далее для каждого пиксела определяют три числа `(R, G, B)`:

- `R` — интенсивность свечения красного,
- `G` — интенсивность свечения зелёного,
- `B` — интенсивность свечения синего (голубого).

Иногда к этим трём числам добавляют "**канал прозрачности**" `A` - это степень прозрачности пикселя. Это число играет роль только при наложении изображений друг на друга.
...
Для того, чтобы отобразить пиксель на экране его разрезают на субпиксели. Каждый субпиксель светит либо красным, либо зелёным, либо синим цветом.

![Пиксели2](images/03-Pixel2.jpeg)

Для кодирования цветов пикселей используют разные схемы.

1. **Представление в виде палитры (устарела)**

    Этот подход аналогичен однобайтовым текстовым кодировкам. Сначала люди составляют "**алфавит цветов**", который называется **палитра**, то есть договариваются каким числам соответствуют какие цвета, а затем используют эти числа для кодирования изображений. Стандартные палитры:

   - CGA (320 x 200 пикселов, 16 цветов; 640 x 200 пикселов, 2 цвета);
   - EGA (640 x 350 пикселов, 16 цветов из палитры в 64 цвета) в компьютерах x286 и x386;
   - VGA (640 x 480 пикселов, 16 цветов и 320 x 200 пикселов, 256 цветов);

    При помощи палитры можно делать эффект [циклического сдвига палитры](https://www.progamer.ru/dev/color-design.htm).

2. **Прямое кодирование цвета (TrueColor)**

    При таком подходе палитры нет, а каждое число из тройки `(R, G, B)` кодируется некоторым числом бит.

    | Число бит | Название | Число бит на каждый канал `(R, G, B, A)` | Число цветов | Комментарий |
    | --- | --- | --- | --- | --- |
    | 8 | 8bit TrueColor | 3, 3, 2, 0 | 256 | в консольных компьютерах |
    | 12 | 12bit TrueColor | 4, 4, 4, 0 | 4096 | в первых цветных телефонах |
    | 15 | HighColor | 5, 5, 5, 0 | 32768 | |
    | 16 | HighColor | 5, 6, 5, 0 | 65536 | «тысячи цветов» (thousands of colors) в системах Macintosh |
    | 24 | TrueColor | 8, 8, 8, 0 | 16.7 млн | Используется сейчас в бытовых мониторах |
    | 32 | TrueColor+Alpha | 8, 8, 8, 8 | 16.7 млн | Используется сейчас в бытовых мониторах |
    | 30, 36, 48 | DeepColor |  | Много | Профессиональная техника |

#### Векторное кодирование изображений

При векторном кодировании изображение представляется в виде набора геометрических фигур и математических формул. В результате для определения цвета точки изображения необходимо подставить координаты этой точки в "формулу", которой закодировано изображение, и получить цвет этой точки в виде трёх чисел `(R,G,B)`.

#### MIDI Кодирование звука
...
#### Импульсно-кодовая модуляция для кодирования звука
...
### 2023-02-18 Количество информации

**Информация** — сведения независимо от формы их представления.

#### Количество информации как число бинарных вопросов

Вопрос называется **бинарным**, если он подразумевает ровно два ответа.

**Определение 1.** Будем говорить, что **количество информации**, содержащееся в некотором сообщении, равно *минимальному* числу бинарных вопросов, ответы на которые позволяют полностью восстановить информацию, переданную в сообщении.

Единицей измерения **количества информации** (также как и единицей измерения длины сообщения) является **1 бит (bit)**.

**Пример 1.** Вы получили сообщение. Известно, что в сообщении передано целое число из отрезка $[0; 7]$. Для однозначного восстановления этого числа достаточно трёх бинарных вопросов, а двух вопросов не хватит. Поэтому количество информации в этом сообщении равно 3 бита.

Предположим теперь, что вам дополнительно стало известно, что передаваемое число чётно? Сколько теперь информации содержится в передаваемом сообщении? Сколько информации вам сообщили, когда сказали, что число чётно?

**Идея.** Количество информации должно при каких-то условиях складываться.

**Пример 2.** Вам сообщили вашу фамилию. Сколько бит информации вы получили? А если сообщили фамилию вашего одноклассника? В след за фамилией вам сообщили имя одноклассника. Сколько дополнительных бит информации вы получили?

**Идея.** Количество информации зависит от того, что уже знает получатель. Если в первом примере получатель не знает понятие целого числа, то вначале он задаст кучу уточняющих вопросов, чтобы определить для себя это понятие.

То есть количество информации можно рассматривать как **меру неожиданности** сообщения для получателя.

**Идея.** Если вам сообщили что-то, что вы уже и так знаете, то вы получили 0 бит информации.

**Идея.** Если в классе много учеников, то сообщение, содержащее фамилию ученика, будет содержать больше информации, чем в случае когда учеников мало. Вывод: количество информации зависит вероятности возникновения события.

#### Количество информации как минимальная длина сообщения

**Определение 2.** Будем говорить, что **количество информации**, содержащееся в некотором сообщении, равно длине *минимальной* последовательности из `0` и `1`, которой можно закодировать данное сообщение.

**Пример 1.** Для того, чтобы закодировать любое целое число из отрезка $[0; 7]$ достаточно трёх бинарных цифр. Поэтому сообщение, содержащее число из этого отрезка, содержит 3 бита информации.

**Пример 2.** Пусть сообщение - это буква из множества `{а, б, в}`. Для кодирования такого сообщения нам надо потратить 2 бита. Но, оказывается, что при кодировании серии таких сообщений можно обойтись и меньшим число бит.

**Пример 2. Предположение 1.** Предположим, что буквы в серии встречаются одинаково часто. Рассмотрим последовательность из двух букв (сочетание из двух подряд идущих символов/букв/токенов/слов называется биграмма). Таких биграмм всего $3\cdot 3 = 9$. Для кодирования одной из девяти биграмм нам понадобится 4 бита. Выигрыша нет, но 9 значительно ближе к 8, чем к 16. Рассмотрим теперь триграммы (последовательность из трёх подряд идущих сообщений). Триграмм всего $3^3 = 27$. Поэтому для кодирования одной триграммы можно использовать 5 бит, так как $2^5 = 32 > 27$. Это на один бит меньше, чем если бы мы кодировали каждую букву отдельно двумя битами.

**Пример 2. Предположение 2.** Предположим, что сообщение `а` встречается в серии в два раза чаще, чем `б`, а сообщение `б` так же часто как и сообщение `в`. Тогда рассмотрим кодирование:
...
Такой код называется префиксным. Если серия содержит $n$ сообщений, то на её кодирование мы в среднем потратим

$$
\frac{n}{2}\cdot 1 + \frac{n}{4}\cdot 2 + \frac{n}{4}\cdot 2 = \frac{3}{2}n
$$

Рассуждая аналогично, можно заключить, что короткий код должно получить то сообщение, которое встречается чаще других. Это подтверждает идею о том, что чем более вероятно сообщение, тем меньше информации оно несёт.

**Пример 2. Предположение 3.** Предположим теперь, что сообщение `в` в серии встречается тогда и только тогда, когда перед ним идёт сообщение `б`. В этом случае оптимальное кодирование выглядит так:
...
Но это только в том случае, если мы будем запоминать предыдущее сообщение. Если же мы будем его забывать, то на кодирование сообщения `в` нам придётся тратить биты.

**Идея**. Предположим, что к нам поступило два сообщения. Если при кодировании второго сообщения мы не использовали информацию из первого, то общее количество полученной информации складывается. В противном случае, если сообщения связаны между собой, то суммарный объём полученной информации может быть существенно меньше (вспоминаем пример про фамилию и имя).

### 2023-03-04 Количество информации

#### Показательная функция и логарифм

Обратим внимание на то, что в двух предыдущих определениях количеством информации являлась степень, в которую следует возвести число 2, чтобы получить нужное число различных вариантов бинарных ответов или кодировок. Такую степень называют логарифм.

**Определение.** Пусть $a > 0$, тогда функция $f(x) = a^x$ называется показательной функцией с основанием $a$.

Вы уже умеете определять значение этой функции для всех $x\in\mathbb{Q}$. Для всех остальных $x\in\mathbb{R}\setminus\mathbb{Q}$ значение этой функции определяется так, чтобы функция была непрерывна. Другими словами, если $\sqrt{2} \approx 1.414$, то $a^{\sqrt{2}} \approx a^{1.414}$.

Простые свойства:

- область определения $D(f) = (-\infty; +\infty) = \mathbb{R}$;
- множество значений $E(f) = (0;+\infty)$, то есть $f(x) = a^x > 0$ для всех $a > 0$ и всех $x\in\mathbb{R}$;

- если $a > 1$, то
  - $f(x) = a^x$ монотонно возрастает;
  - $f(-\infty) = a^{-\infty} = 0$;
  - $f(+\infty) = a^{+\infty} = +\infty$;

- если $a < 1$, то
  - $f(x) = a^x$ монотонно убывает;
  - $f(-\infty) = a^{-\infty} = +\infty$;
  - $f(+\infty) = a^{+\infty} = 0$;

- $f(x+y) = a^{x+y} = a^x a^y = f(x)f(y)$;

**Определение.** Пусть $a > 0$ и $a\neq 1$ и $x > 0$. Логарифмом числа $x$ по основанию $a$ (обозначение $\log_a x$) называют степень, в которую следует возвести число $a$, чтобы получить число $x$.

Другими словами, $a^{\log_a x} = x$. Из этого следует, что функция $g(x) = \log_a x$ является обратной к показательной функции $f(x) = a^x$ и называется логарифмической функцией.

Простые свойства:

- график функции $g(x) = \log_a x$ симметричен графику функции $f(x) = a^x$ относительно прямой $y = x$.

- область определения $D(g) = (0; +\infty)$;

- множество значений $E(g) = (-\infty; +\infty) = \mathbb{R}$;

- $f(g(x)) = x$, поэтому $a^{\log_a x} = x$;

- $g(f(x)) = x$, поэтому $\log_a (a^x) = x$, в частности $\log_a a = 1$;

- если $a > 1$, то
  - $g(x) = \log_a x$ монотонно возрастает;
  - $g(+0) = \log_a (+0) = -\infty$;
  - $g(+\infty) = \log_a (+\infty) = +\infty$;

- если $a < 1$, то
  - $g(x) = \log_a x$ монотонно убывает;
  - $g(+0) = \log_a (+0) = +\infty$;
  - $g(+\infty) = \log_a (+\infty) = -\infty$;

- пусть $x > 0$ и $y > 0$, тогда найдём такие $u$ и $v$, что $x = a^u$ и $y = a^v$, и запишем $g(xy) = \log_a (xy) = \log_a (a^u a^v) = \log_a (a^{u+v}) = u + v = \log_a x + \log_a y = g(x) + g(y)$.

**Пример.** Если в сообщении передано случайное целое число из отрезка $[0; 7]$, то мы получили $3 = \log_2 8$ бит информации. Здесь 8 - это число различных вариантов сообщения. Логично обобщить этот вывод на случай, когда сообщение случайно выбрано из множества размера $N$. В этом случае мы получим $\log_2 N$ бит информации.

#### Формула Хартли (1928 г)

Статья на [википедии](https://ru.wikipedia.org/wiki/Формула_Хартли).

**Определение.** Пусть алфавит некоторого языка содержит $N$ символов (мощность алфавита). Тогда сообщение длины $K$, закодированное в этом языке, содержит
...
В этом определении неявно предполагается, что символы сообщения появляются независимо друг от друга.

#### Определение по Шеннону

Статья на [википедии](https://ru.wikipedia.org/wiki/Информационная_энтропия#Определение_по_Шеннону).

[Клод Шеннон](https://ru.wikipedia.org/wiki/Шеннон,_Клод) предположил, что прирост информации равен утраченной неопределённости, и задал требования к её измерению. Эти требования можно переформулировать в виде аксиом:

- $I \geqslant 0$ - количество (собственной) информации $I$ *неотрицательно*.

- $I = I(p)$ - количество информации $I$ в сообщении *непрерывно зависит* от вероятности $p$ появления этого сообщения (и не зависит от содержания сообщения); то есть изменение вероятности на малую величину должно вызывать малое изменение количества информации.

- $I(p_1 p_2) = I(p_1) + I(p_2)$ - если одно сообщение можно разбить на две независимых части, то количество информации в исходном сообщении равно сумме количества информации в этих частях.

- $I(\frac{1}{2}) = 1$ - результат броска монеты содержит один бит информации.
...
1. Пусть $n\in\mathbb{N}$. Тогда $I(2^{-n}) = I(\frac{1}{2}\dots\frac{1}{2}) = I(\frac{1}{2}) + \dots + I(\frac{1}{2}) = n I(\frac{1}{2}) = n$.

2. Пусть $k\in\mathbb{N}$ и $I(2^{-\frac{1}{k}}) = x$. Тогда $1 = I(2^{-1}) = I((2^{-\frac{1}{k}})^k) = k x$. Следовательно, $x = \frac{1}{k}$.

3. $I(2^{-\frac{n}{k}}) = I((2^{-\frac{1}{k}})^n) = n I(2^{-\frac{1}{k}}) = \frac{n}{k}.$

4. Обозначим $p = 2^{-\frac{n}{k}}$. Тогда $\frac{n}{k} = -\log_2 p$. Следовательно, $I(p) = -\log_2 p$.

5. Так как количество информации непрерывно зависит от вероятности возникновения события, то формула $I(p) = -\log_2 p$ верна для любого $p\in (0; 1]$.

**Следствие.**

- $I(1) = -\log_2 1 = 0$ - если сообщение не случайно, то оно не несёт информации.

- количество информации монотонно убывает с ростом вероятности $p$: чем событие более вероятно (более ожидаемо) тем меньше информации оно несёт.

- Если сообщение - это целое число равновероятно выбранное из отрезка $[0;7]$, то вероятность появления любого числа из этого отрезка равна $\frac{1}{8}$. Следовательно, такое сообщение несёт $I(\frac{1}{8}) = -\log_2 \frac{1}{8} = 3$ бита информации.

**Определение.** Энтропия $H$ потока независимых сообщений - это среднее количество собственной информации $I$, получаемое с каждым сообщением потока.

То есть, чтобы получить энтропию $H$ потока независимых сообщений надо просуммировать количество информации вот всех сообщений и поделить эту сумму на число сообщений в потоке.

### 2023-03-11 Количество информации. Задачи

1. В корзине лежат 8 красных, 4 синих и 2 белых шара. Из корзины извлекли шар. Найдите количество информации, которое мы получаем для каждого цвета шара. Найдите энтропию этого эксперимента.

2. В условиях первой задачи первый шар положили обратно, перемешали и извлекли новый шар. Найдите количество информации, которое мы получаем для каждой пары цветов. Найдите энтропию этого эксперимента.

3. Задача 2, но первый шар не возвращали.

4. В алфавите племени МУМУ всего 6 знаков: буквы А, У, М, К, точка и пробел. В популярном романе МУМУКА содержится 10000 знаков, из них: А - 4000, У - 1000, М - 2000, К - 1500, точек - 500, пробелов - 1000. Знаки в тексте встречаются независимо. Какой объём информации содержит книга? Предложите наиболее оптимальное бинарное кодирование букв. Сколько бит будет содержать книга при таком кодировании?

5. Архиватор - это программа, которая перекодирует сообщение, пытаясь уменьшить его размер. Идеальный архиватор - это архиватор, который уменьшает размер любого сообщения хотя бы на 1 бит. Докажите, что идеального архиватора не существует.

### 2023-03-18 Разбор контрольной работы

### 2023-04-08 Практика. Подсчёт количества информации в файле

#### Правила

- Если вы сдаёте задание на уроке, то получаете за него указанную оценку.
- Если вы не успеваете сдать задание на уроке, то его надо будет сдать к следующей субботе 2023-04-15, но оценка будет на `1` балл ниже (но если вы сегодня готовитесь к матбою, то для вас правило минус балл не работает).
- Если вы не сдаёте задание к следующей субботе, то получаете оценку `1` в журнал.

#### Задание на 4

Напишите программу, которая вычисляет количество информации в файле и энтропию одного знака алфавита. При подсчёте количества информации следует полагать, что алфавит языка состоит из всех встреченных в тексте символов и эти символы встречаются независимо друг от друга. Имя файла программа должна спрашивать у пользователя.

Замечание. Для вычисления количества информации необходимо для каждого знака алфавита посчитать сколько раз этот знак встретился в тексте. Для этого можно использовать [*словарь*](https://pythonworld.ru/tipy-dannyx-v-python/slovari-dict-funkcii-i-metody-slovarej.html) - это коллекция, которая содержит отображение из произвольного ключа в значение, связанное с этим ключом. Создать пустой словарь можно так:

```Python
dictionary = {}
```

Положить в словарь пару (key, value) можно так:

```Python
dictionary[key] = value
```

Увеличить на `1` значение в словаре можно так:
...
См. задание на 4, но при вычислении количества информации следует считать, что алфавит языка состоит не из одиночных символов, а из последовательностей символов фиксированной длины: биграмм, триграмм, тетраграмм, и так далее. Эту длину следует спросить у пользователя. Например, если пользователь указал длину знака алфавита - 3 символа, то в тексте `"Мама мыла раму"` будет 5 знаков: `"Мам"`, `"а м"`, `"ыла"`, `" ра"`, `"му "`.

### 2023-04-15 Разборы контрольной и практики

### 2023-04-22 Решаем пробник ОГЭ

Переходите по ссылке и выполняйте тот вариант, который вам укажет преподаватель.

Телефоном пользоваться нельзя. Во время выполнения задания в браузере должна быть открыта ровно одна вкладка с текстом задания. Кнопку **Проверить** нажимать нельзя.

Первая оценка ставится за выполнение заданий с кратким ответом:
...
Вторая оценка - пятёрка - дополнительно выставляется в журнал, если выполнено хотя бы одно задание из части "Задания с развернутым ответом".

### 2023-05-13 Алгоритмы построения оптимального префиксного кодирования

#### Алгоритм Шеннона

Основная статья на [википедии](https://ru.wikipedia.org/wiki/Алгоритм_Шеннона).

1. Символы $a_i$, $i = 1,\dots,n$, алфавита $A$ выписывают по убыванию вероятностей $p_i$.
2. Для каждого символа $a_i$ вычисляется его количество информации, округлённое вверх: $l_i = \lceil -\log_2 p_i \rceil$.
3. В качестве кода символа $a_i$ используются $l_i$ первых цифр после запятой из двоичного представления кумулятивной вероятности $p_1 + \dots + p_{i-1}$.

Этот метод был первым в своём роде, но он является менее оптимальным, чем метод Шеннона−Фано.

**Замечание.** Из того, что $l_i = \lceil -\log_2 p_i \rceil$ следует неравенство

$$
l_1 - 1 < -\log_2 p_i \leqslant l_i;
$$

$$
-l_1 \leqslant \log_2 p_i < -l_i + 1;
$$

$$
2^{-l_1} \leqslant p_i < 2^{-l_i + 1};
$$

$$
1 \leqslant p_i\cdot 2^{l_i} < 2.
$$

А из этого неравенства следует, что $l_i$ это номер первого ненулевого бита после запятой в бинарном разложении числа $p_i$. Следовательно, прибавление $p_i$ к сумме $p_1 + \dots + p_{i-1}$ точно бедт менять хотя бы один бит из первых $l_i$ битов суммы. Поэтому код Шеннона будет префиксным.

**Пример, при котором алгоритм Шеннона даёт не оптимальное кодировнаие.** Пусть в алфавите два символа `a` и `b` таких, что вероятность символа `a` равна $1-2^{-100}$, а вероятность символа `b` равна $2^{-100}$. В этом случае код символа `a` будет стоять из $l_a = \lceil -\log_2 (1-2^{-100}) \rceil = 1$ бита, а код символа `b` будет состоять из $l_b = \lceil -\log_2 2^{-100} \rceil = 100$ битов, что очевидно не оптимально.

**Особенность реализации.** При реализации алгоритма надо иметь ввиду, что операции с плавающей точкой выполняются с ошибками округления. Поэтому все вычисления надо выполнять с фиксированной точностью в $L = \max l_i$ битов. Этого можно добиться, если умножеить все вероятности на $2^L$, округлить их вниз до ближайшего целого, и все вычисления проводить с этими целыми числами.


#### Алгоритм Шеннона−Фано

Основная статья на [википедии](https://ru.wikipedia.org/wiki/Алгоритм_Шеннона_—_Фано).

1. Символы алфавита $A$ выписывают по убыванию вероятностей.
2. Символы алфавита делят на две части, суммарные вероятности символов которых максимально близки друг другу.
...
Алгоритм Шеннона−Фано строит дерево префиксного кодирования сверху вниз: сначала множество всех символов алфавита разбивается на два подмножества - левое и правое. Все символы левого подмножества отправляются в левое поддерево и дописывают бит `0` к своему коду, а все символы правого множества отправляются в правое поддерево и дописывают бит `1` к своему коду.

Разбиение на левое и правое подмножества следует выбирать так, чтобы вероятность попадание в каждое их них была примерно равна $\frac{1}{2}$. Это некоторое эмпирическое правило, которое вроде как должно выполнятся для оптимального кодирования.

Построить требуемое разбиение на левое и правое подмножества - довольно трудоёмкая задача. Это разновидность задачи о рюкзаке.
...
Основная статья на [википедии](https://ru.wikipedia.org/wiki/Код_Хаффмана).
...
Алгоритма Хоффмана строит оптимальное префиксное дерево снизу вверх, опираясь на два утверждения, которые справедливы для оптимального префиксного кодирования.

1. Символ с наименьшей вероятностью должен иметь самый длинный код. Более того. Если вероятность символа $a$ меньше вероятности символа $b$, то код символа $a$ должен быть не короче кода символа $b$. В противном случае коды символов $a$ и $b$ можно поменять местами и получить более оптимальное кодирование.

2. Если у символа $x$ самый длинный код, то найдётся ещё один символ $y$ с кодом такой же длины. Докажем от противного. Пусть $l_x$ - длина кода символа $x$, а $y$ - самый длинный символ из оставшися, и длина его кода $l_y$. Так как кодирование символов префиксное, то первых $l_y$ бит кода символа $x$ не будут префиксом никакого другого символа. Поэтому для кодирования символа $x$ достаточно использовать первых $l_y$ бит его кода. Мы уменьшили код символа $x$, значит первоначальное кодироввание не было оптимальным. Противоречие.

#### Арифметическое кодирование

Основная статья на [википедии](https://ru.wikipedia.org/wiki/Арифметическое_кодирование).
...
1. Определение позиционной системы счисления. Приведите пример представления числа в трёх различных системах счисления.

2. Сформулируйте признак делимости на $q^n$ в системе счисления с основанием $q$.

3. Пусть $q\equiv_m 1$. Сформулируйте признак делимости на $m$ в системе счисления с основанием $q$.

4. Пусть $q\equiv_m -1$. Сформулируйте признак делимости на $m$ в системе счисления с основанием $q$.

5. Докажите, что конечная десятичная дробь всегда будет периодической в системе счисления $q$, если $q$ взаимно просто с $10$.

6. Быстрый перевод чисел из двоичной в шестнадцатиричную систему и обратно.

**Информация**

7. Определения информации, языка, сообщения, алфавита и кодирования информации (без потери и с потерей).

**Измерение информации**

8. Определение размера сообщения. Единицы измерения сообщений в ЭВМ.

9. Упаковка байтов в инструкциях к CPU.

10. Единицы измерения скорости передачи информации.

**Кодирование**

11. Кодирование целых чисел без знака.

12. Порядок записи байтов числа.

13. Кодирование целых чисел со знаком.

14. Кодирование дробных чисел (числа с плавающей запятой) на примере стандарта `float32`.

15. Формулы для вычисления числа значащих десятичных знаков, наименьшего и наибольшего положительных значений при кодировании с плавающей запятой.

16. Специальные значения при кодировании с плавающей запятой и операции над ними. Пример кодирования специальных значений в `float16`.

17. Однобайтовое кодирование текстов: кодовые страницы (таблицы), преимущества и недостатки. Приведите названия трёх однобайтовых кодировок и область их применения.

18. Что такое `Unicode`, `UTF-8`, `UTF-16`, `UTF-32`. В чём принципиальное отличие `Unicode` от однобайтовых кодировок.

19. Правила при работе с текстовыми файлами при написании программ.

20. Растровое кодирование изображений. Понятие пикселя и субпикселя.

21. Кодирование цвета: палитра, прямое кодирование цвета.

22. Векторное кодирование изображений.

23. MIDI Кодирование звука.

24. Импульсно-кодовая модуляция для кодирования звука.

**Количество информации**

25. Количество информации как число бинарных вопросов. Примеры.

26. Количество информации как минимальная длина сообщения. Примеры.

27. Определение показательной функции и её свойства.

28. Определение логарифмической функции и её свойства.

29. Формула Хартли и её недостатки.

30. Определение количества информации по Шеннону с доказательством.

31. Следствия из определения количества информации по Шеннону. Энтропия потока сообщений.

32. Докажите, что идеального архиватора не существует.

33. Алгоритм Шеннона построения префиксного кодирования.

34. Алгоритм Шеннона−Фано построения префиксного кодирования.

35. Алгоритм Хаффмана построения префиксного кодирования.

**Python**

36. Фрагмент кода, который читает из стандартного ввода целое число с приглашением `Введите целое число:`.

37. Фрагмент кода, который печатает на экране значение переменной `foo` типа `float` с точностью до 4-х знаков после запятой.

38. Пример функции `char_to_digit`, которая преобразует символ шестнадцатиричной цифры в её числовое значение.

39. Вырезание подстроки из строки, поиск подстроки в строке, число символов в строке.

40. Получение Unicode-кода символа и преобразование Unicode-кода в символ.

41. Создание статичного словаря и получение из него значения на примере словаря `BIN_TO_OCT`.

42. Фрагмент кода, который добавляет в словарь новую пару `(key, value)`.

43. Фрагмент кода, который увеличивает на `1` значение, соответствующее ключу `key` или сопоставляет этому ключу значение `1`.

44. Фрагмент кода, который перебирает все символы в строке.

45. Проверка того, что символ является буквой.

46. Пример программы, которая читает всё содержимое файла в одну строковую переменную `content`.

47. Пример программы, которая читает всё содержимое файла построчно.

48. Фрагмент кода, в котором создаётся функция `foo`: функция `foo` принимает на вход целое число и возвращает строку `чёт`, если число чётное и `нечет` - если нечётное. Приведите пример вызова этой функции и печати результата на экране.

49. Фрагмент кода, который собирает из списка строк `str_list` одну строку, используя в качестве разделителя запятую.

50. Фрагмент кода, который создаёт список целых чисел от `2` до `n`, где `n` - целочисленная переменная, `n >= 2`.

51. Фрагмент кода, который печатает чётные целые числа от `2` до `n`, где `n` - целочисленная переменная, `n >= 2`.

52. Фрагмент кода, который выбрасывает исключение, если число `n` больше `100`.

53. Отличие между операциями `/` и `//`.
